...
def kill_all():
    ...
    process = subprocess.Popen(r"curl http://localhost:6800/listjobs.json?project=ARGUS", stdout=subprocess.PIPE, shell = True)
    l = str(process.stdout.read()).split(",")
    for c in l:
        if "id" in c and "pid" not in c:
            ID = str(c.split(":")[-1]).replace('"', '').lstrip()
            subprocess.run(r"curl http://localhost:6800/cancel.json -d project=ARGUS -d job={}".format(ID), shell = True)
            print("Killed job: ", ID)
